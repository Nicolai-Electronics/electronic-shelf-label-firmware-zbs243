...
__bit boardGetPsk(uint8_t __xdata *psk)
{
	return flashRead(FLASH_INFOPAGE_ADDR + 0x10 + 8, psk, 16);
}

__bit boardGetPan(uint8_t __xdata *pan)
{
	return flashRead(FLASH_INFOPAGE_ADDR + 0x10 + 8 + 16, pan, 2);
}
